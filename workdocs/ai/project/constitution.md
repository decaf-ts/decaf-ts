...
*   **ClientBasedService Consumers:** Classes that extend `ClientBasedService` are usually extension points rather than complete end-user services. Consuming code should normally subclass them, override `initialize()` to build the config object first, and pass that config to `super`/the base lifecycle. The config is typically derived from Decaf's `Environment` object. In tests, either subclass and decorate the test double, or run the test knowing the class will not be injected by `@service()` and will not be initialized by `Service.boot()`.
