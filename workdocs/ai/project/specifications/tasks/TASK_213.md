...
## 2. Objectives
*   [x] Add plan node/edge/layer types and `GraphExecutionPlan` (with `incomingByNode`/`outgoingByNode` maps).
*   [x] Add `GraphRelationResolver` normalizing workflow relations, resolving workflow boundary aliases (`$workflow`, `workflow`, `graph`, `workflow.name`), and validating endpoints/ports.
*   [x] Add `GraphExecutionPlanner` with `plan(workflow)` that resolves nodes/relations, validates unique IDs, builds maps, detects cycles, and produces topological layers.
*   [x] Reject arbitrary non-loop cycles with `GraphCycleError`.
*   [x] Treat workflow boundary as non-executable; treat loop nodes as ordinary executable nodes at the containing workflow level.
...
## 4. Verification Plan
**Automated Tests:**
*   [x] Unit Test: `tests/unit/graph/GraphRelationResolver.test.ts`
*   [x] Unit Test: `tests/unit/graph/GraphExecutionPlanner.test.ts`
