...
-   [2026-06-08] - Final stabilization: fixed CLI module to boot `RamAdapter` + `TaskService` when `taskMode=true`, corrected `cli-live-app.module.cjs` to use `lib/cjs/` paths and export `AppModule` directly (not wrapped in an object), and added defensive adapter pre-registration in `DecafMigrationModule.migrate()` to survive the `for-nano` module hazard. Test `cli-migrate.multi-adapter.integration.test.ts` passes end-to-end.
