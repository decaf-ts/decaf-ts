...
- **Priority:** Medium
- **Goal:** Port every executable script in the decaf-ts repo to TypeScript as decaf-cli command modules (auto-picked-up, invoked as `decaf <command> ...`) under full decaf conventions (`@service()`, `logCtx`, decaf errors, zero `console.*`, async/await): root `bin/` helpers, the 30-package duplicated `bin/tag-release.sh` and 7-package `bin/sync-codex.sh` copies replaced by exactly one shared TypeScript implementation, `with-ai` executable scripts and docker entrypoints, and root `package.json` orchestration entries that wrap executable scripts now invoking the CLI; watchers/long-running sync become foreground CLI commands with signal-clean SIGINT/SIGTERM shutdown (no daemonization); parity baseline per script (no functional redesign; deviations only where decaf conventions require, each documented); `mcp-server/bin` excluded (retires under DECAF-51); live `with-ai/agents/` and `with-ai/skills/` untouched.
- **Status:** Approved — specification initialized 2026-09-04 for domain root [SAA-650](/SAA/issues/SAA-650); Product Manager scope approved (new specification, distinct from DECAF-51 — not an amendment); CTO technical review completed 2026-09-04 on [SAA-654](/SAA/issues/SAA-654) — **approved with conditions**, all conditions folded into the record the same day under [SAA-659](/SAA/issues/SAA-659) (ten inventory corrections, binding "executable script" definition, the five former open questions decided: docker full port with ≤ ~10-line zero-logic shims only where containers require; hosting in `@decaf-ts/utils` as `decaf utils <command>` / `decaf with-ai <command>` kebab-case preserving legacy base names; mandatory `decaf utils credentials` resolver for every credential-touching port incl. the PM in-scope ruling on `do-install`/`set-git-auth`/`set-dev`; foreground watchers with manual restart; one shared `decaf utils tag-release` + existing `decaf release chain|dispatch` with the chain contract unchanged); two new risks and the DECAF-51 coordination guard recorded; implementation decomposition by the PM pending.
- **Link:** [Specification Details](./specifications/DECAF_52.md)
- **Tasks:**
  - [ ] Pending decomposition by the domain-root owner (Product Manager) into implementation children — specification approved; root/per-package/utils work proceeds immediately, with-ai module ports sequenced per the DECAF-51 coordination guard.
...
- DECAF-52: ⏳ Port All Repo Scripts to TypeScript decaf CLI Commands (spec initialized 2026-09-04 under [SAA-650](/SAA/issues/SAA-650); PM scope approved — new spec distinct from DECAF-51; CTO technical review approved with conditions 2026-09-04 on [SAA-654](/SAA/issues/SAA-654), conditions folded under [SAA-659](/SAA/issues/SAA-659) — spec approved; implementation pending PM decomposition)
