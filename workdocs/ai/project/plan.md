...
- **Status:** In Progress — approved scope (phases P1–P7) delivered, user-approved-committed, and verified 2026-09-02/03 under implementation child [SAA-394](/SAA/issues/SAA-394) (QA [SAA-577](/SAA/issues/SAA-577); security [SAA-576](/SAA/issues/SAA-576)→[SAA-595](/SAA/issues/SAA-595); parameter controls [SAA-596](/SAA/issues/SAA-596)); board reopened corrective scope 2026-09-04 — `for-angular` lib⇄`integrations` boundary, shared-types placement, session adapter, node layout — refactor in progress under [SAA-704](/SAA/issues/SAA-704); completion documentation recorded 2026-09-05 under [SAA-648](/SAA/issues/SAA-648) (Paperclip authoritative; this line is a snapshot as of 2026-09-05).
- **Link:** [Specification Details](./specifications/DECAF_50.md)
- **Tasks:**
  - [x] Phase 1 — Shared contracts: canonical document/instance/binding/manifest/parameter/visibility/schema types, builder, serializer, decorated compiler (`ui-decorators/src/graph/document` + `catalog`) — [SAA-394](/SAA/issues/SAA-394).
  - [x] Phase 2 — Backend catalogue: `integrations/src/graph/engine/catalog`, registration validation, built-in manifest+executor pairs, catalogue/dynamic-method HTTP APIs — [SAA-394](/SAA/issues/SAA-394).
  - [x] Phase 3 — Engine resolution: document validation, `GraphResolvedWorkflow`, catalogue plan nodes, parameters/inputs separation, output validation, nested documents — [SAA-394](/SAA/issues/SAA-394).
  - [x] Phase 4 — Angular canonical store: `GraphWorkflowDocumentStore`, `GraphDiagramAdapter`, manifest palette, document commands, schema-driven forms, save/history/autosave integration — [SAA-394](/SAA/issues/SAA-394).
  - [x] Phase 5 — Persistence: canonical document endpoints (`PUT/GET /graph/workflows/{id}`, `POST /graph/workflows/validate`), lossless legacy conversion, ownership — [SAA-394](/SAA/issues/SAA-394).
  - [x] Phase 6 — Run lifecycle: `POST /graph/runs` (202 before completion), status/result/cancel, authorized replayable run-scoped SSE, Angular run clients — [SAA-394](/SAA/issues/SAA-394).
  - [x] Phase 7 — Cutover: run the exact editor document, reject inline definitions, remove constructor palette discovery and independent `GraphNodeConfigStore`, remove raw-definition planning and the `GRAPH_CANONICAL_DOCUMENT_ENABLED` flag, complete the 12-step E2E, update docs — [SAA-394](/SAA/issues/SAA-394).
  - [x] Follow-up — backend security hardening ([SAA-576](/SAA/issues/SAA-576) findings F1–F8) — [SAA-595](/SAA/issues/SAA-595).
  - [x] Follow-up — frontend parameter-controls coverage (dynamic options, resource locator, credential picker, registered-UI fallback) — [SAA-596](/SAA/issues/SAA-596).
  - [ ] Reopen — corrective boundary refactor (`for-angular` lib off `integrations`, shared types/models in `ui-decorators/graph`, session adapter, example nodes, node layout, build+tests) — [SAA-704](/SAA/issues/SAA-704), in progress.

---

## DECAF-51 — with-ai MCP/CLI — mcp-server Redesign and Port
- **Priority:** Medium
- **Goal:** Retire `./mcp-server` and redesign its full capability surface into `./with-ai`: with-ai stays authoritative for skills/agents/markdown (coverage validated, not assumed); TS functionality becomes decaf-cli commands; MCP boot is a fully detached CLI command exposing only jira/xray + common tooling under `src/mcp/{jira,xray,common}` with per-module self-registration; per-request client boot with lazy credential loading (user hard requirement); latest MCP SDK + new transports; multi-project Jira access (`JIRA__PROJECT_KEY` restriction removed); industry-standard MCP policy with clearly displayed destructive tools; `@decaf-ts/crypto`-based build-time encryption of all AI-value content (never committed); CLI installs harness skills via symlinks to decrypted md files in the installed package; TS + inspector testing against the production artifact; live Jira/Xray tests idempotent, self-reverting, with loud cleanup-failure warnings and minimal live noise; no functionality lost, measured at the capability level; no commands and no prompts in with-ai (2026-09-02 user amendment) — MCP module surface is tools/templates only, ticket templates are single-sourced from `with-ai/skills/common/maintain-domain-docs/assets/` with no workdocs duplicates, and `with-ai/workdocs/prompts/**` plus `with-ai/workdocs/templates/**` are removed (live `with-ai/agents/` and `with-ai/skills/` untouched).
- **Status:** COMPLETED — all phases delivered and verified (Phase 1 legacy inventory + catalog coverage [SAA-566](/SAA/issues/SAA-566); Phase 2 CLI core + detached MCP boot + crypto pipeline + skill installer [SAA-567](/SAA/issues/SAA-567); Phase 3 jira module with multi-project allowlist + per-request clients [SAA-568](/SAA/issues/SAA-568); Phase 4 xray module [SAA-569](/SAA/issues/SAA-569); Phase 5 common module + MCP policy + package conformance [SAA-570](/SAA/issues/SAA-570); Phase 6 TS + inspector test suites + live idempotent tests [SAA-571](/SAA/issues/SAA-571), live-jira green with self-reversion evidence); amendments implemented ([SAA-614](/SAA/issues/SAA-614), recorded under [SAA-615](/SAA/issues/SAA-615)); commit-gate cycles green through the final re-gates ([SAA-655](/SAA/issues/SAA-655)–[SAA-658](/SAA/issues/SAA-658) → [SAA-664](/SAA/issues/SAA-664) → [SAA-665](/SAA/issues/SAA-665)–[SAA-668](/SAA/issues/SAA-668) → [SAA-675](/SAA/issues/SAA-675) → [SAA-676](/SAA/issues/SAA-676)–[SAA-678](/SAA/issues/SAA-678), all green); completion recorded 2026-09-04 under [SAA-684](/SAA/issues/SAA-684) for domain root [SAA-560](/SAA/issues/SAA-560); reopened-scope items 1–7 (2026-09-05) delivered, verified, and security-gated ([SAA-707](/SAA/issues/SAA-707)/[SAA-708](/SAA/issues/SAA-708)/[SAA-709](/SAA/issues/SAA-709); security gate [SAA-728](/SAA/issues/SAA-728) PASS with one low finding routed to the root) — phase-2 completion recorded 2026-09-05 under [SAA-729](/SAA/issues/SAA-729); the first single user-approved commit landed 2026-09-05 (repo root `293ab32` + with-ai submodule pin `c9fde50`, not pushed); the reopened-scope low-priority pair is delivered **uncommitted** — [SAA-732](/SAA/issues/SAA-732) (`./utils` AgileTest equivalent) and [SAA-733](/SAA/issues/SAA-733) (`agents` preset, superseding the [SAA-615](/SAA/issues/SAA-615) no-prompts amendment for that preset only) — pending the next commit gate, after which the root reaches `done`. Non-blocking follow-ups: [SAA-669](/SAA/issues/SAA-669) (source-JSDoc coverage B3–B7), [SAA-670](/SAA/issues/SAA-670) (CLI/MCP usage docs); xray live suites deferred per user direction until xray-capable credentials exist; a real out-of-band `ENCRYPTION_KEY` is required before any decrypt-sensitive release build.
- **Link:** [Specification Details](./specifications/DECAF_51.md)
- **Tasks:**
  - [x] Phase 1 — Legacy inventory + catalog coverage ([SAA-566](/SAA/issues/SAA-566)) — 100% legacy-surface inventory (Req-8); "no uncovered capability remains".
  - [x] Phase 2 — CLI core + detached MCP boot + crypto pipeline + skill installer ([SAA-567](/SAA/issues/SAA-567)).
  - [x] Phase 3 — jira module: `JIRA_PROJECT_KEYS` multi-project allowlist + per-request clients ([SAA-568](/SAA/issues/SAA-568)).
  - [x] Phase 4 — xray module ([SAA-569](/SAA/issues/SAA-569)).
  - [x] Phase 5 — common module + MCP policy + package conformance ([SAA-570](/SAA/issues/SAA-570)).
  - [x] Phase 6 — TS + inspector test suites + live idempotent tests ([SAA-571](/SAA/issues/SAA-571)) — live jira green with self-reversion evidence; xray live created but deferred per user direction.
  - [x] [SAA-614](/SAA/issues/SAA-614) — remove `with-ai/workdocs/prompts/**` and `with-ai/workdocs/templates/**` plus every reference; `jira-ticket-create` sources ticket templates from `skills/common/maintain-domain-docs/assets/` (2026-09-02 user amendment, recorded under [SAA-615](/SAA/issues/SAA-615)).
  - [x] Reopened scope 1 — mcp-server parity port (vs `mcp-server@afd9dfa`) + provisioning fixes; EncryptionService `ClientBasedService` conformance; `decaf-mcp` bin; `--preset` + global `--read-only`; AgileTest preset ([SAA-707](/SAA/issues/SAA-707)).
  - [x] Reopened scope 2 — deployment-command validation (17 CLI commands) + additive `paperclip-plugin/` scaffold ([SAA-708](/SAA/issues/SAA-708)).
  - [x] Reopened scope 3 — test reorganization into unit / integration (live jira, from source) / e2e (dist via inspector) tiers with single-dist builds ([SAA-709](/SAA/issues/SAA-709); implementation [SAA-723](/SAA/issues/SAA-723); unit extension QA [SAA-710](/SAA/issues/SAA-710); fix round [SAA-726](/SAA/issues/SAA-726)).
  - [x] Security gate — reopened-scope delta verified PASS; one low finding (L1, `bin/tag-release.sh` npm-publish default flip + false comment) routed to the root ([SAA-728](/SAA/issues/SAA-728)).
  - [x] Low-priority pair — `./utils` AgileTest equivalent for the xray tooling ([SAA-732](/SAA/issues/SAA-732)) + `agents` preset in the with-ai MCP ([SAA-733](/SAA/issues/SAA-733); supersedes [SAA-615](/SAA/issues/SAA-615) for the `agents` preset only) — delivered 2026-09-05, uncommitted pending the next commit gate (recorded under [SAA-750](/SAA/issues/SAA-750)).
  - [ ] [SAA-669](/SAA/issues/SAA-669) — follow-up (non-blocking): source-JSDoc coverage pass (B3–B7).
  - [ ] [SAA-670](/SAA/issues/SAA-670) — follow-up (non-blocking): CLI/MCP usage docs + stale `workdocs/4-Description.md`.

---

## DECAF-52 — Port Common Repo Scripts to TypeScript decaf CLI Commands (scope narrowed 2026-09-09)
- **Priority:** Medium
- **Goal:** *(amended 2026-09-09 per board ruling)* Port to TypeScript inside the `utils` package only the scripts **common to all or most modules** — typically each package's `./bin` (the duplicated per-package `tag-release.sh` / `sync-codex.sh` copies) plus the root `bin/` repo-operation equivalents (`run-all`, `npm-link`, `npm-token`, `bundle`, `build-scripts`, `update-scripts`, `collect-slogans`, `modules`, `copy-ai-docs`) — consolidating repository operations so they run anywhere, under full decaf conventions (`@service()`, `logCtx`, decaf errors, zero `console.*`, async/await); **every utils command must have its `./cli` wrapper** so packages invoke it via the decaf CLI (AC-7). One shared implementation replaces the duplicated copies (AC-4); `update-scripts` stops redistributing retired copies. Explicit non-goals: with-ai-specific `bin/*.mjs`, `with-ai/docker/**/*.sh` entrypoints, `with-ai/scripts/jira-mcp-smoke.ts`, one-off per-package scripts, and `mcp-server/bin` (stays retired under DECAF-51). Root `package.json` orchestration entries (script phase vs utils CLI commands) are an open decision point — product recommendation: map to utils CLI where the entry is a common repo operation — not a hard mandate. Supersedes the prior port-everything framing (superseded areas marked inline in the spec).
- **Status:** Approved, then **amended 2026-09-09 — scope narrowed** per board ruling; amendment recorded by [SAA-1082](/SAA/issues/SAA-1082) (milestone child of [SAA-1081](/SAA/issues/SAA-1081)) under domain root [SAA-650](/SAA/issues/SAA-650). Prior approval lineage unchanged: PM scope approved 2026-09-04 (new specification, distinct from DECAF-51); CTO technical review approved with conditions 2026-09-04 on [SAA-654](/SAA/issues/SAA-654), conditions folded under [SAA-659](/SAA/issues/SAA-659). **Implementation children must NOT be created yet — the board user reviews the revised specification first.** Amendment is independent of the blocked deliverable work under [SAA-696](/SAA/issues/SAA-696).
- **Link:** [Specification Details](./specifications/DECAF_52.md)
- **Tasks:**
  - [ ] **On hold — board user review of the revised specification.** No implementation children until the board user accepts the amended DECAF-52 record; decomposition then by the domain-root owner (Product Manager).

---

## DECAF-53 — Editable Dashboard Component (for-angular)
- **Priority:** Medium
- **Goal:** Ship a reusable Dynamic Layout/Dashboard component in for-angular's `lib` (working name `DashboardComponent`, selector `ngx-decaf-dashboard`) plus a demo page in the for-angular `app`: runtime-composable dashboards where a builder selects decorated Crud-based internal components from a palette, configures them through their own create/update models, sizes/positions them on a columns/rows grid (dotted-border drop preview while the mouse is held, snap-to-position on drag and border-resize), and deletes them via a top-right `x` confirmation screen; the dashboard itself extends the Crud component pattern (create/update/read/delete) and uses decaf's `@uilayout`/`@uilayoutprop` metadata plus `ModelBuilder` to dynamically generate renderable UI model classes; the saved composition serializes through existing decaf model/repository machinery; pure decaf (decorated classes and handlers wherever possible); v1 non-goals: no new server-side APIs/persistence changes, no undo/redo, no collaborative editing, no mobile-specific interaction design, no nested dashboards.
- **Status:** Approved — specification initialized 2026-09-06 for domain root [SAA-909](/SAA/issues/SAA-909) (milestone [SAA-912](/SAA/issues/SAA-912)); Product Manager product scope approved; CTO technical review completed 2026-09-07 on [SAA-913](/SAA/issues/SAA-913) — **approve-with-changes** (confidence 0.9, context-manifest pinned at `160f18317344faf16828b785882207a1e7c0cacc`); all required changes and both PM decisions folded into the record the same day under [SAA-921](/SAA/issues/SAA-921) (Crud base = `NgxFormDirective` per the `CrudFormComponent` precedent; selectable-component decorator lives in `ui-decorators` with a registry, palette service in for-angular; no AOT blocker — hard rule: palette bounded by the `@Dynamic()` registry, no runtime string→component creation, deterministic generated-class names; grid collision = reject-overlap, no auto-reflow in v1; serialization = persistable decaf model via existing repository machinery; demo persistence = existing local/in-browser adapter; demo on a distinct `dashboard-builder` route — the static demo keeps `app-dashboard`; security hard rules elevated to acceptance level — tag whitelist on load, validator-checked config, no persisted handlers, no `bypassSecurityTrust*`; i18n translation keys only with mandated Playwright i18n screenshot tests; CTO delivery note: staged decomposition recommended — core composition + CRUD + rendering first, then interaction polish); implementation of [SAA-908](/SAA/issues/SAA-908) on hold by board directive; decomposition dispatched separately by the CEO after completion.
- **Link:** [Specification Details](./specifications/DECAF_53.md)
- **Tasks:**
  - [ ] Pending decomposition — dispatched separately by the CEO after specification completion, then broken down by the domain-root owner (Product Manager) into implementation children; the specification is approved (CTO review approve-with-changes 2026-09-07, folded under [SAA-921](/SAA/issues/SAA-921)); staged decomposition recommended per the CTO delivery note.
...
- DECAF-50: ⏳ Canonical Workflow Documents, Backend Node Catalogue & Remote Graph Execution (approved scope P1–P7 delivered and verified 2026-09-02/03 under [SAA-385](/SAA/issues/SAA-385) via SAA-394 + QA SAA-577 + security SAA-576/595 + parameter controls SAA-596; board reopened corrective scope 2026-09-04 — refactor in progress under [SAA-704](/SAA/issues/SAA-704); completion documentation recorded 2026-09-05 under [SAA-648](/SAA/issues/SAA-648); residuals: SAA-596 functional findings, owner-column migration note, supersession references + HowToUse/custom-node docs)
- DECAF-51: ✅ with-ai MCP/CLI — mcp-server Redesign and Port (all phases delivered and verified; amendments implemented under SAA-614; commit-gate cycles green through final re-gates SAA-676–678; completion recorded 2026-09-04 under SAA-684 for domain root SAA-560; reopened-scope items 1–7 delivered, verified, and security-gated 2026-09-05 (SAA-707/708/709; security gate SAA-728 PASS, one low finding routed to the root) — phase-2 completion recorded under SAA-729; first single user-approved commit landed 2026-09-05 (root 293ab32 + with-ai submodule c9fde50, not pushed); low-priority pair SAA-732/SAA-733 delivered uncommitted pending the next commit gate — agents preset supersedes the SAA-615 no-prompts amendment for that preset only (recorded under SAA-750); non-blocking follow-ups SAA-669/SAA-670; xray live deferred; real ENCRYPTION_KEY required before decrypt-sensitive release builds)
- DECAF-52: ⏳ Port Common Repo Scripts to TypeScript decaf CLI Commands (spec initialized 2026-09-04 under [SAA-650](/SAA/issues/SAA-650), approved with CTO conditions [SAA-654](/SAA/issues/SAA-654)/[SAA-659](/SAA/issues/SAA-659); **amended 2026-09-09 under [SAA-1082](/SAA/issues/SAA-1082) — scope narrowed per board ruling to scripts common to all/most modules in `utils` with mandatory `./cli` wrappers; implementation children on hold pending board review of the revised spec**; independent of blocked [SAA-696](/SAA/issues/SAA-696))
- DECAF-53: ⏳ Editable Dashboard Component (for-angular) (spec initialized 2026-09-06 under [SAA-909](/SAA/issues/SAA-909); PM product scope approved; CTO technical review approve-with-changes 2026-09-07 on [SAA-913](/SAA/issues/SAA-913) — required changes and both PM decisions (reject-overlap; local/in-browser demo persistence) folded under [SAA-921](/SAA/issues/SAA-921), spec approved; implementation of [SAA-908](/SAA/issues/SAA-908) on hold by board directive; decomposition dispatched separately by the CEO after completion)
